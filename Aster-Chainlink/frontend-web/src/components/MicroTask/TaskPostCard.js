...
export default function TaskPostCard({ post, index, maticPrice }) {
    ...
    <Grid item xs={5} >
      <Card sx={{ position: 'relative' }}>
            <CardContent>
                ...
                >
                    Pay: { (post.total_price / post.number_of_labelers) + " MATIC"}
                </Typography>
                <Box sx={{ height: 10 }} />
                ...
                <Typography variant="body2">Contract Address: <br/>
                  <a target="_blank" href={`https://mumbai.polygonscan.com/address/${post.contract_address}`}>{post.contract_address}</a>
                </Typography>
            </CardContent>
