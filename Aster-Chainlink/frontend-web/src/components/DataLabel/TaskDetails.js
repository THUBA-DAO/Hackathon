...
function TaskDetails({ task_id}) {
  ...
  const fetchTask = async () => {
    const task_response = await axios.get(`https://us-central1-aster-chainlink.cloudfunctions.net/api/task/${task_id}`);
    const submission_response = await axios.get(`https://us-central1-aster-chainlink.cloudfunctions.net/api/task/labels/${task_id}`);
        ...
        <DialogContent>
        {
            ...
            <div>
                ...
                <Typography variant="subtitle1">Contract Address:
                  <a target="_blank" href={`https://mumbai.polygonscan.com/address/${task.contract_id}`}>{task.contract_id}</a>
                </Typography>
                <Box sx={{ height: 20 }} />
                <Typography variant="string">Total Price: {task.total_price + " MATIC"}</Typography>
                <Box sx={{ height: 3 }} />
                <Typography variant="string">Number of Labelers: {task.number_of_labelers}</Typography>
                <Box sx={{ height: 3 }} />
                <Typography variant="string">Price per Labeler: { (task.total_price / task.number_of_labelers) + " MATIC"}</Typography>
                <Box sx={{ height: 30 }} />
